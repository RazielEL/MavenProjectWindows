...
public class APIWorkflowSteps {
    ...
    public void the_employee_we_are_getting_having_id_must_match_with_the_globally_stored_employee_id(String empID) {
       String tempID = response.jsonPath().getString(empID);
        ...
        System.out.println("Generated before: " + employee_id);
    }

    @Then("the retrieved data at {string} object matches the data used to create the employee having emloyee id {string}")
    public void the_retrieved_data_at_object_matches_the_data_used_to_create_the_employee_having_emloyee_id(String empObject, String resEmpID, DataTable dataTable) {
    ...
    @Given("a request is prepared to create an employee via json payload {string}, {string}, {string}, {string}, {string}, {string}, {string}")
    public void a_request_is_prepared_to_create_an_employee_via_json_payload(String firstname, String lastname, String middlename, String gender, String dob, String status, String jobTitle) {

        request = given().header(APIConstants.HEADER_CONTENT_TYPE, APIConstants.HEADER_CONTENT_TYPE_VALUE).
                header(APIConstants.HEADER_AUTHORIZATION, GenerateTokenSteps.token).body(APIPayloadConstants.createEmployeeDynamic(firstname, lastname, middlename, gender, dob, status, jobTitle));
