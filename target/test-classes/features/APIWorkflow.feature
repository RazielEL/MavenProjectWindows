Feature: This feature covers all the API related scenarios
  ...
  Scenario: Get the newly created employee
    ...
    And the retrieved data at "employee" object matches the data used to create the employee having emloyee id "employee.employee_id"
    |emp_firstname|emp_lastname|emp_middle_name|emp_gender|emp_birthday|emp_status|emp_job_title|
    |Arelka       |Stanislawowska|Ewelina      |Female         |1987-06-11  |Probation |QA           |
  ...
  Scenario: Adding an employee using dynamic scenario
    Given a request is prepared to create an employee via json payload "Arelka", "Stanislawowska", "Ewelina", "F", "1987-06-11", "Probation", "QA"
    When a POST call is made to create an employee
    Then the status code for the created employee is 201
    And the employee created contains key "Message" and value "Employee Created"
    And the employee id "Employee.employee_id" is stored as a global variable to be used for other calls
